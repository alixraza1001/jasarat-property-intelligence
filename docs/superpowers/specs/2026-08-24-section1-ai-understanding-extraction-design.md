...
\`\`\`
*Note: CNIC values are explicitly excluded from this schema to protect privacy.*
...
**Decision C (Schema Mapping):** The application will define a `ProviderModelInterface`. External JSON schemas will be passed to the provider (e.g., OmniRoute or direct SDK). The raw output is validated via Zod, then deterministically mapped to the Canonical Domain Contract. This isolates provider hallucination from business logic.

## 4. Model Evaluation Harness
A script `pnpm run eval:models` will process the `.fixtures/` dataset against configured providers. It will output machine-readable NDJSON and a human-readable markdown summary, capturing latencies, provider failures, and correctness scores.
...
Raw JSON is untrusted. Zod handles structural validation. If validation fails, a structured `JasaratAIValidationError` is returned. Provider errors (timeout, rate limit) map to a normalized taxonomy (`RATE_LIMITED`, `PROVIDER_UNAVAILABLE`, etc.).

## 11. Confidence Strategy
**Decision E (Confidence Scoring):** We do not trust model self-reported confidence. Computed deterministically:
- Base: 100.
- `-10` for missing optional fields (e.g., price, size).
- `-25` for missing critical fields (e.g., plot number).
- `-20` if `UNCERTAIN` flags are raised by the model.
- Thresholds: `< 75` = Review Required, `75 - 89` = Review Recommended, `>= 90` = High Confidence.
...
- **Field Accuracy:** Exact match for plot/phone; normalized match for society/size.

## 13. Model Selection Strategy
**Decision F (Model Candidates):**
We prioritize free/low-cost constraints with strong multimodal capabilities:
1. **Google Gemini 1.5 Flash:** Primary candidate. Exceptionally fast, very low cost, excellent Urdu/multimodal capabilities.
2. **OpenAI GPT-4o-mini:** Secondary candidate for comparison.
3. **Google Gemini 1.5 Pro:** Fallback for highly complex pages if Flash fails.

**Decision A (Architecture):** Evaluate a **Single-Pass Multimodal** approach first using Gemini 1.5 Flash due to its low latency and massive context window. If the structured output hallucination rate is too high, fallback to a **Hybrid** pipeline (Relevance -> Targeted Extraction).
